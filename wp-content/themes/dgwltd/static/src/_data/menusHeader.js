...
const CACHE_KEY = 'menusHeader';
const CACHE_FOLDER = path.resolve("./.cache");
const CACHE_FILE = "menusHeader.json";

async function requestMenusHeader() {
    ...
    try {
        ...
        menusHeader = menusHeader.concat(response.data.menu.menuItems.nodes);
    ...
    const menusFormatted = menusHeader.map((item) => {
        return {
            id: item.id,
            label: item.label,
            url: item.url,
...
module.exports = requestMenusHeader;