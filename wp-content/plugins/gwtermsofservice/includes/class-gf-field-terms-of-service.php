...
class GF_Field_Terms_Of_Service extends GF_Field_Checkbox {
	...
	public function get_form_inline_script_on_page_render( $form ) {
		...
		foreach ( $form['fields'] as $field ) {
			...
			$script .= 'gwTosScroll( jQuery( "#gw_terms_' . $field['id'] . '"), ' . $field['id'] . ', ' . $form['id'] . ' );

                jQuery("#gw_terms_' . $field['id'] . '").scroll(function(){
	...
	public function get_terms( $form ) {
		...
		if ( is_callable( array( 'GWPreviewConfirmation', 'preview_replace_variables' ) ) ) {
			...
			$terms = GFCommon::replace_variables_prepopulate( $terms );
		}
	...
	public function key( $key ) {
		return gp_terms_of_service()->perk->key( $key );
	}
