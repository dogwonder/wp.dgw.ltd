...
class GF_RECAPTCHA extends GFAddOn {
	...
	public function init() {
		...
		add_filter( 'gform_pre_render', array( $this, 'add_recaptcha_field' ) );
		add_filter( 'gform_pre_validation', array( $this, 'add_recaptcha_field' ) );
	...
	public function add_recaptcha_field( $form ) {
		if (
			...
			|| $this->is_disabled_by_form_setting( $form )
			|| count( GFAPI::get_fields_by_type( $form, 'recaptcha' ) ) > 0
		) {
			return $form;
		}

		$next_field = GFFormsModel::get_next_field_id( $form['fields'] );

		$form['fields'][] = GF_Fields::create(
			array(
				'type'       => 'recaptcha',
				'id'         => $next_field++,
				'label'      => __( 'reCAPTCHA v3', 'gravityformsrecaptcha' ),
				'formId'     => rgar( $form, 'id' ),
				'visibility' => 'hidden',
			)
		);
	...
	public function update_entry_meta( $key, $entry, $form ) {
		...
		$recaptcha_field = GFAPI::get_fields_by_type( $form, 'recaptcha' );

		if ( count( $recaptcha_field ) !== 1 ) {
			return;
		}
	...
	public function check_for_spam_entry( $is_spam, $form, $entry ) {
		if ( $is_spam || $this->is_disabled_by_form_setting( $form ) || ! $this->initialize_api() ) {
			return $is_spam;
		}

		return (float) $this->get_score_from_entry( $entry ) <= $this->get_spam_score_threshold();
	}
	...
	private function get_spam_score_threshold() {
		return (float) $this->get_plugin_setting( 'score_threshold_v3', 0.5 );
	}
	...
	private function is_disabled_by_form_setting( $form ) {
		return '1' === rgar( $this->get_form_settings( $form ), 'disable-recaptchav3' );
	}
	...
	public function validate_submission( $submission_data ) {
		$this->log_debug( __METHOD__ . '(): validating form submission.' );
