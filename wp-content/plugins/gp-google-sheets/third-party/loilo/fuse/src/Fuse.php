...
namespace GP_Google_Sheets\Dependencies\Fuse;

use GP_Google_Sheets\Dependencies\Fuse\Bitap\Bitap;
use function GP_Google_Sheets\Dependencies\Fuse\Helpers\deep_value;
use function GP_Google_Sheets\Dependencies\Fuse\Helpers\is_list;
class Fuse
{
    ...
    {
        $this->options = \array_merge(['location' => 0, 'distance' => 100, 'threshold' => 0.6, 'maxPatternLength' => 32, 'caseSensitive' => \false, 'tokenSeparator' => ' +', 'findAllMatches' => \false, 'minMatchCharLength' => 1, 'id' => null, 'keys' => [], 'shouldSort' => \true, 'getFn' => '\GP_Google_Sheets\\Dependencies\\Fuse\\Helpers\\get', 'sortFn' => function ($a, $b) {
            if ($a['score'] === $b['score']) {
        ...
        }, 'tokenize' => \false, 'matchAllTokens' => \false, 'includeMatches' => \false, 'includeScore' => \false, 'verbose' => \false], $options);
        ...
        if (\sizeof($keys) > 0 && \is_string($keys[0])) {
            ...
            foreach ($keys as $key) {
                ...
                if ($keyWeight < 0 || $keyWeight > 1) {
                    throw new \Exception('"weight" property in key must bein the range of [0, 1)');
                }
                if (\is_null($highest)) {
                    $highest = $keyWeight;
                } else {
    ...
    protected function innerSearch($tokenSearchers = [], $fullSearcher = null)
    {
        ...
        if (\is_string($list[0] ?? null)) {
            ...
            for ($i = 0, $len = \sizeof($list); $i < $len; $i++) {
                $this->analyze(['key' => '', 'value' => $list[$i], 'record' => $i, 'index' => $i], $resultMap, $results, $tokenSearchers, $fullSearcher);
            }
            return $results;
        }
        ...
        foreach ($list as $i => $item) {
            ...
            foreach ($this->keyNames as $key) {
                $this->analyze(['key' => $key, 'value' => $this->options['getFn']($item, $key), 'record' => $item, 'index' => $i], $resultMap, $results, $tokenSearchers, $fullSearcher);
            }
        }
        return $results;
    }
    ...
    {
        $query = \array_merge(['key' => null, 'arrayIndex' => -1, 'value' => null, 'record' => null, 'index' => null], $query);
        $key = $query['key'];
        $search = function ($arrayIndex, $value, $record, $index) use($key, $fullSearcher, $tokenSearchers, &$resultMap, &$search, &$results) {
            ...
            if (\is_string($value)) {
                ...
                $checkTextMatches = $this->options['tokenize'] && $this->options['matchAllTokens'] ? $numTextMatches >= \sizeof($tokenSearchers) : \true;
                $this->log("\nCheck Matches: ", $checkTextMatches);
                // If a match is found, add the item to <rawResults>, including its score
                if (($exists || $mainSearchResult['isMatch']) && $checkTextMatches) {
                    $_searchResult = ['key' => $key, 'arrayIndex' => $arrayIndex, 'value' => $value, 'score' => $finalScore];
                    ...
                    if (!\is_null($existingResult)) {
                        ...
                        $resultMap[$index] = ['item' => $record, 'output' => [$_searchResult]];
    ...
    protected function computeScore(&$results)
    {
        ...
        foreach ($results as &$result) {
            ...
            for ($j = 0; $j < $scoreLen; $j++) {
                ...
                $score = $item['score'] === 0 && $hasWeights && $weights[$key] > 0 ? \defined('PHP_FLOAT_EPSILON') ? \PHP_FLOAT_EPSILON : 1.0E-5 : $item['score'];
        ...
        if ($this->options['includeMatches']) {
            $transformers[] = function ($result, &$data) {
                ...
                for ($i = 0, $len = \sizeof($output); $i < $len; $i++) {
                    ...
                    $obj = ['indices' => $item['matchedIndices'], 'value' => $item['value']];
                    if ($item['key']) {
                        $obj['key'] = $item['key'];
                    }
